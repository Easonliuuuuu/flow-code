...
export class Engine {
    ...
    checkBudgets() {
        ...
        for (const id of this.nodeAborts.keys()) {
            ...
            if (spent > limit) {
                this.nodeBudgetStops.set(id, `node token budget exhausted: ${spent} tokens spent of ${limit} allowed`);
        ...
        const visit = (nodeId) => {
            for (const depId of this.wf.graph.directDependencies(nodeId)) {
                if (seen.has(depId))
                    continue;
                seen.add(depId);
                if (this.nodeById(depId).type.contextTransparent)
                    visit(depId);
    ...
    workingDirFor(id) {
        ...
        while (queue.length > 0) {
            ...
            if (dep.type.id === 'worktree-agent') {
                const output = this.store.node(depId).output;
                if (output?.convergedDir)
                    return output.convergedDir;
            }
     ...
     */
    recordRetryReason(targetId, sourceId) {
    ...
    fireLoopback(sourceId) {
        ...
        if (!firable) {
            ...
            this.store.setStatus(sourceId, 'error', `${detail ? `${detail} — ` : ''}loop-back attempt limit reached: ${exhausted}`);
            return false;
        }
        ...
        try {
            for await (const event of executor(ctx)) {
                if (event.type === 'status') {
                    ...
                    if (!parsed.success) {
                        throw new Error(`node output does not match the ${node.type.id} output schema: ${parsed.error.issues
    ...
    startEligible() {
        ...
        for (const id of this.wf.order) {
            ...
            if (needsLock && this.mainTreeLockHolder !== null)
                continue;
            if (needsLock)
                this.mainTreeLockHolder = id;
            const promise = this.runNode(node).finally(() => {
                if (this.mainTreeLockHolder === id)
                    this.mainTreeLockHolder = null;
                this.running.delete(id);
            });
            this.running.set(id, promise);
            // Starting a Discuss node freezes further starts this pass.
            if (node.type.id === 'discuss')
                break;
        }
    }
    async run() {
