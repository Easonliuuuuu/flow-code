 ...
 */
export function ellipsis(frame) {
 ...
 */
export function plannedSummary(node) {
    const c = config(node);
    switch (node.type.id) {
        case 'discuss':
        ...
        case 'validate':
            return firstLine(typeof c['instructions'] === 'string' ? c['instructions'] : 'check the work matches intent');
        case 'review':
            return firstLine(typeof c['instructions'] === 'string' ? c['instructions'] : 'critique the pending diff');
        case 'git-ops': {
            const push = c['push'];
            if (push?.remote && push.branch)
                return `commit + push → ${push.remote}/${push.branch}`;
 ...
 * green dots.
 */
export function outcomeSummary(node, output) {
    ...
    switch (node.type.id) {
        ...
        case 'git-ops': {
            if (o['pushed'] === true)
                return `pushed → ${String(o['remote'])}/${String(o['branch'])}`;
            if (o['committed'] === true)
                return `committed ${String(o['commit'] ?? '').slice(0, 7)}`.trimEnd();
