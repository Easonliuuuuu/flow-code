...
export function dockedRect(bounds, height) {
 ...
 */
export function dockedLayout(bounds, headerRows) {
    // Always leave at least one canvas row, however short the terminal is.
    const maxHeight = Math.max(1, bounds.rows - headerRows - 1);
    const height = Math.min(Math.max(MIN_PANEL_HEIGHT, Math.floor(bounds.rows * DOCKED_HEIGHT_RATIO)), maxHeight);
 ...
 */
export function hitTestPanel(rect, x, y) {
    if (x < rect.x || x >= rect.x + rect.w || y < rect.y || y >= rect.y + rect.h)
        return null;
    if (x >= rect.x + rect.w - RESIZE_GRIP_W && y >= rect.y + rect.h - RESIZE_GRIP_H)
        return 'resize';
    ...
    if (onTop || onBottom || onLeft || onRight || onTitleBar)
        return 'move';
    return null;
}
...
export function applyPanelMove(origin, dx, dy, bounds) {
...
export function applyPanelResize(origin, dx, dy, bounds) {
 ...
 */
export function tailWindow(total, visible, pin) {
 ...
 */
export function pinAfterScroll(win, deltaRows) {
    const nextStart = win.start - deltaRows;
    if (nextStart >= win.maxScroll)
        return null;
