...
export async function callOpenAiCompatChat(opts) {
    ...
    const request = (apiKey) => fetch(`${opts.baseUrl}/chat/completions`, {
        method: 'POST',
        headers: {
        ...
        for (let attempt = 0;; attempt++) {
            ...
            try {
                ...
                if (opts.signal?.aborted)
                    throw new OpenAiCompatApiError(`${opts.baseUrl} request aborted`);
                lastDetail = `request failed: ${err instanceof Error ? err.message : String(err)}`;
                if (attempt >= MAX_RETRIES) {
                    if (hasNextKey) {
                        console.warn(`flow-code: ${opts.baseUrl} still failing (${lastDetail}) after ${MAX_RETRIES} retries; rotating to the next API key…`);
                        break;
                    }
            ...
            if (attempt >= MAX_RETRIES) {
                if (hasNextKey) {
                    console.warn(`flow-code: ${opts.baseUrl} still ${res.status} ${res.statusText} after ${MAX_RETRIES} retries; rotating to the next API key…`);
                    break;
                }
                throw new OpenAiCompatApiError(`${opts.baseUrl} request failed: ${lastDetail}`);
            }
            ...
            console.warn(`flow-code: ${opts.baseUrl} returned ${res.status} ${res.statusText}; retrying in ${Math.round(delay)}ms…`);
            await sleep(delay, opts.signal);
        }
    }
    throw new OpenAiCompatApiError(`${opts.baseUrl} request failed: ${lastDetail}`);
