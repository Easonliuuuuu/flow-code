...
import { callOpenAiCompatChat } from './openaiCompatClient.js';
import { editFileTool, globTool, grepTool, listDirTool, readFileTool, runShellTool, writeFileTool, } from './nvidiaToolExec.js';
/** Fails a run rather than looping forever against a model that never stops calling tools. */
const MAX_TOOL_LOOP_ITERATIONS = 40;
async function executeTool(name, args, workingDir, shellEnv, signal) {
    switch (name) {
        case 'read_file':
...
function buildInterceptor(req, store) {
...
async function runTurn(config, messages, req, apiKeys, interceptor) {
    ...
    for (let iteration = 0; iteration < MAX_TOOL_LOOP_ITERATIONS; iteration++) {
        ...
        for (const call of message.tool_calls) {
            ...
            if (decision.behavior === 'deny') {
                ...
                try {
                    const { text, exitStatus } = await executeTool(call.function.name, args, req.workingDir, shellEnv, req.signal);
                    resultText = text;
    ...
    throw new Error(`${config.label} agent session for node "${req.nodeId}" exceeded ${MAX_TOOL_LOOP_ITERATIONS} tool-call iterations without finishing.`);
}
/**
 ...
 */
export class OpenAiCompatSessionRunner {
    config;
    constructor(config) {
        this.config = config;
    }
    ...
    async openInteractive(req, store) {
        ...
        const config = this.config;
        const interceptor = buildInterceptor(req, store);
        const messages = [{ role: 'system', content: systemPrompt(req) }];
...
//# sourceMappingURL=openaiCompatRunner.js.map