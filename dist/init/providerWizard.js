 ...
 */
export async function runProviderWizard(repoRoot) {
    console.log('\nflow-code: pick the provider and model that will back every agent-driven node.\n');
    const provider = await selectFromList(PROVIDERS.map((p) => ({ label: p.label, value: p.id })), { prompt: 'Provider:' });
    if (!provider)
        return undefined;
    ...
    if (info.apiKeyEnvVar) {
        ...
        console.log('  No Claude credentials detected yet (ANTHROPIC_API_KEY, CLAUDE_CODE_OAUTH_TOKEN, or a `claude` CLI ' +
...
//# sourceMappingURL=providerWizard.js.map