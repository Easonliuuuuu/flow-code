...
function editNode(path, nodeId, mutate) {
    ...
    try {
        ...
        throw new WorkflowWriteError(`could not read ${path}: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    try {
        ...
        throw new WorkflowWriteError(`refusing to write ${path}: the edited workflow would no longer load — ${reason}`);
    }
