 ...
 */
export const BINARY_OPERATORS = ['==', '!=', '>=', '<=', '>', '<', 'contains'];
...
function parseLiteral(text) {
    ...
    if (text.length > 0 && !Number.isNaN(asNumber))
        return asNumber;
    throw new ConditionParseError(`\`${text}\` is not a valid value — use a quoted string, a number, true, false, or null`);
}
/**
...
export function parseCondition(source) {
    ...
    if (text.length === 0)
        throw new ConditionParseError('condition is empty');
    ...
    for (const operator of BINARY_OPERATORS) {
        ...
        if (lhs.length === 0)
            throw new ConditionParseError(`nothing to compare before \`${operator}\``);
        if (rhs.length === 0)
            throw new ConditionParseError(`nothing to compare after \`${operator}\``);
        return { ...parseReference(lhs, source), operator, value: parseLiteral(rhs), source: text };
    }
    throw new ConditionParseError(`no operator found — expected one of ${[...BINARY_OPERATORS, ...UNARY_OPERATORS]
        .map((o) => `\`${o}\``)
        .join(', ')}`);
}
/**
 ...
 */
function indexOfOutsideQuotes(text, needle) {
    let quote = null;
...
function parseReference(text, source) {
    const match = REFERENCE.exec(text);
    if (!match) {
        throw new ConditionParseError(`\`${text}\` is not a \`<node>.<field>\` reference (in \`${source}\`)`);
    }
...
export function resolvePath(output, path) {
    let current = output;
