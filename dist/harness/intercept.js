...
import { ALWAYS_ALLOWED_TOOLS, ALWAYS_DENIED_TOOLS, EDIT_TOOLS, EXEC_TOOLS, READ_TOOLS, } from './compile.js';
...
export function outsideWorkingDir(workingDir, candidate) {
...
const DENIED_SET = new Set(ALWAYS_DENIED_TOOLS);
const ALLOWED_SET = new Set(ALWAYS_ALLOWED_TOOLS);
export function createInterceptor(opts) {
    ...
    function decide(toolName, input, callOpts) {
        ...
        if (EXEC_SET.has(toolName)) {
            ...
            if (toolName !== 'Bash')
                return { behavior: 'allow' };
            if (callOpts?.blockedPath !== undefined &&
