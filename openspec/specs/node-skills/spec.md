# node-skills Specification

## Purpose

Defines how reusable skills — directories carrying a `SKILL.md` — are discovered across project, user, and plugin roots, attached to agent-driven nodes in the workflow file, resolved and reported before a run starts, and composed into a node's session provider-agnostically without widening the node's capability envelope. Also defines which node types are interactive and how a non-interactive session that ends by asking a question fails legibly.
