# session-status-line Specification

## Purpose

Defines how a live run is summarized as bounded text for a surface flow-code does not own — a host agent's status bar, a shell prompt, a tmux row. Covers what the summary must answer (what the run needs, how far it is, what it has spent), how it degrades as the available width shrinks, how a run whose driver has died is reported, how guarantees a run's enforcement tier never provided are kept from being implied, and the guarantee that reading a run never disturbs the process driving it.
