# first-run-demo Specification

## Purpose

Defines what a stranger can see before committing anything: `flow-code try`, a zero-repository, zero-configuration, zero-credential command that runs the real default graph — the shipped engine, executors, gates, and UI — against a seeded temporary repository, with every agent session scripted. It exists to demonstrate the two moments that distinguish flow-code from a chat window with a todo list — a failing test looping back and recovering, and nothing reaching git without approval — before a user has spent a token or configured a provider.
