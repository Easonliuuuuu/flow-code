# node-model-selection Specification

## Purpose

Defines per-node model overrides: how a node's effective model is resolved, how the user picks a model for the focused node from the terminal canvas, how a chosen model is persisted to the workflow file, when a model change takes effect during a run, and how an override is surfaced on the graph.
