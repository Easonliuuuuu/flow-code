...
Defines how a project's test commands are determined — filesystem heuristics first, a `read`-only agent session as fallback — how proposals are confirmed by the user and persisted into the Test node's config in `.flow-code/workflow.yaml`, and the constraints that keep Test node execution deterministic by default.
