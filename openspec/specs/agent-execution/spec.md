...
- **WHEN** the loaded workflow contains at least one node that wants an agent session, and no provider can be resolved — no per-repo choice saved by `flow-code init`, no provider API key in the environment, and no CLI login to fall back on
- **THEN** the system SHALL fail before starting any node, naming the environment variables that would satisfy it and pointing at `flow-code init`, and SHALL NOT have created any worktree or modified the repository
...
### Requirement: Provider-backed node execution
The system SHALL execute every agent-driven node through a single `SessionRunner` implementation selected from the project's configured provider, rather than routing by node type. Every session SHALL be scoped to that node's config and working directory, rather than shelling out to an interactive CLI session.

#### Scenario: Implement node runs
- **WHEN** the DAG executor starts an Implement node whose dependencies are satisfied
- **THEN** the system SHALL start an agent session via the run's runner, with that node type's role prompt, the node's configured instructions, and the node's working directory as its working root, and mark the node `running`

#### Scenario: Every agent node uses the configured provider
- **WHEN** the DAG executor starts any node that wants an agent session — Discuss, Implement, Validate, Review, Git-ops, a Worktree-Agent instance, or a Test/Approval-Gate node with its optional agent step enabled
- **THEN** the system SHALL run that session against the provider resolved for the run, using that node's configured model or, if unset, the run's default model, or, if that is also unset, the runner's built-in default model

#### Scenario: Interactive and headless sessions share the runner
- **WHEN** a Discuss node opens an interactive session and another node later runs a headless one
- **THEN** both SHALL be served by the same runner, so the provider a project configures governs the whole run rather than part of it
...
- **WHEN** the same node type and capability set execute once via the Claude Agent SDK-backed runner and once via an OpenAI-compatible runner
