# guest-agent-instructions Specification

## Purpose

Defines the agent-facing instructions generated from a project's own `.flow-code/workflow.yaml` — what the host agent is told about the graph it is expected to walk, where those instructions are installed for each host surface, and how instructions that no longer match the workflow are detected rather than silently followed.
