...
- New default model for NVIDIA-routed nodes: `meta/llama-3.1-70b-instruct` (verified against the live API during implementation — see design.md; `meta/llama-3.3-70b-instruct` was the original pick but hangs indefinitely on this account/tier). The existing per-node `config.model` override continues to work, now selecting an NVIDIA model id instead of a Claude one for NVIDIA-routed node types.
