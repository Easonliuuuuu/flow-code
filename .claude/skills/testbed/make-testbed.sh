...
# this makes.
#
# Usage: make-testbed.sh [--dest PATH] [--shape wide|tall|tiny] [--no-build]
...
cat > "$MARKER" <<'EOF'
Generated by flow-code's `testbed` skill. Everything here is disposable —
the generator deletes and recreates this directory, and refuses to touch a
directory that does not contain this file.
EOF
...
EOF
  ...
  tiny)
    ...
    # The baseline for "does this look right at rest".
    cat > .flow-code/workflow.yaml <<'EOF'
settings:
  model: sonnet
...
echo
echo "testbed ready: $DEST  (shape: $SHAPE)"
echo
echo "  cd $DEST"
