...
export const executeDiscuss: NodeExecutor = async function* (ctx) {
  ...
  try {
    ...
    ctx.ports.discuss.begin(ctx.node.id, config.topic);
    ...
    ctx.ports.discuss.postAssistant(ctx.node.id, await session.send(opening));

    for (;;) {
      const userText = await ctx.ports.discuss.nextUserMessage(ctx.node.id);
      if (userText === null) break;
      ctx.ports.discuss.postAssistant(ctx.node.id, await session.send(userText));
    }
