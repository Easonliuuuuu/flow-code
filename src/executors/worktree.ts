...
function instancePrompt(
  ...
  index: number,
): { instanceId: string; prompt: string; model?: string } {
...
export class ConvergenceConflictError extends Error {
  constructor(readonly conflictingFiles: string[]) {
    super(`merge conflict at convergence in: ${conflictingFiles.join(', ')}`);
    this.name = 'ConvergenceConflictError';
  }
}

export const executeWorktreeAgent: NodeExecutor = async function* (ctx) {
  yield { type: 'status', status: 'running' };
  const config = ctx.node.config as WorktreeAgentConfig;
  const runShort = ctx.runId.slice(0, 8);

  const results: InstanceResult[] = [];
  const runInstance = async (index: number): Promise<void> => {
  ...
  if (results.every((r) => r.status === 'error')) {
    throw new Error(
      `all worktree instances failed: ${results.map((r) => `${r.instanceId}: ${r.summary}`).join('; ')}`,
    );
  }
  ...
  for (const id of selected) {
    if (!selectable.has(id)) throw new Error(`selected instance ${id} did not complete`);
  }
  ...
  for (const result of results) {
    if (result.dir === convergence.dir) continue;
    try {
      ...
      ctx.store.appendLiveOutput(
        ctx.node.id,
        `warning: could not remove worktree ${result.dir}: ${err instanceof Error ? err.message : err}\n`,
      );
    }
  }
