 ...
 */
export const executeSpec: NodeExecutor = async function* (ctx: ExecuteContext) {
  yield { type: 'status', status: 'running' };
  const config = ctx.node.config as SpecConfig;
  ...
  if (config.acceptanceCriteria !== undefined && config.acceptanceCriteria.length > 0) {
    ...
    const prompt =
      `${upstreamPreamble(ctx.upstream)}## Specification task\n\n` +
