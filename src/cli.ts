...
import { listNodeTypes, PLACEHOLDER_TEST_COMMAND, type TestConfig } from './registry/index.js';
...
async function cmdInit(args: string[]): Promise<void> {
  ...
  if (justScaffolded) {
    console.log(
      "  Test command: left as the scaffolded placeholder — `flow-code run` will offer to detect it " +
 ...
 */
export async function resolveTestPlaceholder(
  repoRoot: string,
  workflowPath: string,
  workflow: Workflow,
  provider: { provider: ProviderId; model?: string } | undefined,
): Promise<boolean> {
  const unresolved = workflow.nodes.some((n) => {
    if (n.type.id !== 'test') return false;
    const commands = (n.config as TestConfig).commands;
  ...
  if (!process.stdin.isTTY) {
    fail(
      'a Test node still has the placeholder command — run `flow-code run` once from an ' +
